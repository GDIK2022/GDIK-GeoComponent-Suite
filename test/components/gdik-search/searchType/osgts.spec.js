...
describe("OpenSearch Geo and Time Extensions", () => {
    ...
    it("should request suggest results with given parameters", async () => {
        const osgts = new OSGTS(options),
            result = await osgts.suggest("Oldenburg");

        expect(fetch).toBeCalledWith("https://suggest?outputformat=json&srsName=epsg:25832&query=Oldenburg&count=5");
    ...
    it("should request suggest results with additional bbox", async () => {
        const osgts = new OSGTS(Object.assign({}, options, {extent: [123, 321, 124, 322]})),
            result = await osgts.suggest("Oldenburg");

        expect(fetch).toBeCalledWith("https://suggest?outputformat=json&bbox=123,321,124,322&srsName=epsg:25832&query=Oldenburg&count=5");
